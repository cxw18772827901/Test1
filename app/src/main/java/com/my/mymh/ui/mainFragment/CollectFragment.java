...
public class CollectFragment extends BaseFragment {
    ...
    private void setData(CollectListRecord collectRecord) {
        ...
        if (collectRecord != null && collectRecord.code == 1000) {
            if (collectRecord.result != null && collectRecord.result.size() > 0) {
                List<CollectListRecord.Result> resultList = filterData(collectRecord.result);
    ...
    private List<CollectListRecord.Result> filterData(List<CollectListRecord.Result> results) {
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            switch (viewType) {
                case Item.TYPE_NORMAL:
                    return new ViewHolder1(LayoutInflater.from(parent.getContext()).inflate(R.layout.collect_item, parent, false));
