...
//                    IntentUtil.startActivity(UpdateListActivity.this, CartoonInfoAndCatalogActivity.class);
                    IntentUtil.startRead(UpdateListActivity.this, result);
                }
            }
        });
    ...
    private void initData() {
        ObjectRequest<LastUpdateCartoonListRecord> response = new ObjectRequest<>(false, LastUpdateCartoonListRecord.Input.buildInput(pageSize + "", ""),
...
//                                page += resultList.size();//get(resultList.size() - 1).id;
    ...
    private void loadData() {
        ObjectRequest<LastUpdateCartoonListRecord> response = new ObjectRequest<>(false, LastUpdateCartoonListRecord.Input.buildInput(pageSize + "", page * pageSize + ""),
                newCartoonListRecord -> {
                    if (null != newCartoonListRecord && 1000 == newCartoonListRecord.code) {
                        if (null != newCartoonListRecord.result && newCartoonListRecord.result.size() > 0) {
                            List<LastUpdateCartoonListRecord.Result> dataList = filterData(newCartoonListRecord.result);
